...
namespace PrositCore {
    ...
    double QBDResourceReservationProbabilitySolver::matrix_prob_ts (int i, int j, int q, 
                                                                    const PrositAux::cdf & p, 
    ...
    ///@param p cdf of the computation time
    ///@param u pmf of the interarrival time
    double QBDResourceReservationProbabilitySolver::matrix_prob_ts_compressed (int i, int j, int q, 
                                                                               const PrositAux::cdf & p, 
                                                                               const PrositAux::pmf & u) {
        #ifdef DEBUG
            cerr << "matrix_prob_ts called" << endl;
    ...
    void QBDResourceReservationProbabilitySolver::pre_process() {
        ...
        if (pre_process_done) {
	       if (verbose_flag)
	       cerr << "pre_process called on a solver that has already generated the matrices for task " << 
                task_descriptor->get_name () << endl;
	       return;
        }
        ...
        if (verbose_flag) {
            if (!Q % step)
	           cerr << "Warning: granularity is not a submultiple of budget in computation of matrix for task " <<
	                   task_descriptor->get_name () << endl;
        }
        ...
        if (compress_flag) {
	       ...
	       if (int (u->get_min () * Q) > c->get_max ())
	           EXC_PRINT_2("Compressed mode not usable: task always finishes before its next activation. Task:",
    ...
    void QBDResourceReservationProbabilitySolver::fill_in_probability_map() {
            ...
            for (pmi = pm->begin(); pmi != pm->end(); pmi++) {
                if (((*pmi).first < unsigned(u->get_min()))&&compress_flag)
        	       cerr << "Deadline " << (*pmi).first*T << " will not be computed for task " <<
                        task_descriptor->get_name() <<" because it used the compressed model" << endl;
